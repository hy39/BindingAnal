...
function [aa_array aa_charge aa_charge_sum] = group_aa_dist_byngs(viruses)
...
%for i=1:length(viruses(:,1))
    v = [viruses(:,iso_index) viruses(:,ngs_index) viruses(:,charge_index)];
    ac = push_data(v,ac);
%end
aa_array = ac;
disp 'done';
...
function [ac]= push_data(v,ac)
    ...
    for i=1:length(v)
        ...
        loc = find(ac(:,1)==yr & ac(:,3)==charge &ac(:,2)==ngs);
