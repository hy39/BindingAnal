...
function [aa_array aa_charge aa_charge_sum] = group_aa_dist_byngs_byage(viruses)
...
%for i=1:length(viruses(:,1))
    v = [viruses(:,age_index) viruses(:,ngs_index) viruses(:,charge_index)];
    ac = push_data(v,ac);
%end
aa_array = ac;
disp 'done';
...
function [ac]= push_data(v,ac)
    ...
    for i=1:length(v(:,1))
        ...
        loc = find(ac(:,1)==age & ac(:,3)==charge &ac(:,2)==ngs);
        
        if(isempty(loc))
            ac = [ac; age ngs charge 1];
        else
            ac(loc,4) = ac(loc,4) + 1;
        end
    end
