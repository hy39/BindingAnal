function [] = plot_net_byage_dummy( age_min, age_max, saveoutput )
p = path
path(p,'..');
p = path
path(p,'../lib');
...
for i=1:length(ranges2(:,1))
    ...
    for i=1:siz
      ...
      amount = ac(i,4);
      if(amount<3)
        plot(age,charge,[color_code(ngs_no-7) 'o'], 'MarkerSize', 3);
        %plot(age,charge,'Bo', 'MarkerSize', 2, 'MarkerFaceColor', 'B');
      elseif(amount<8)
        plot(age,charge,[color_code(ngs_no-7) 'o'], 'MarkerSize', 6);
      else
    ...
    if exist('saveoutput')
        if strcmp(saveoutput,'s')
            save(['h3n2_ny_net_byage' num2str(age_min) '-' num2str(age_max) '_dummy_pvalue.mat'], 'pvalue', 'ptable', 'age_min', 'age_max');
        end
    end
