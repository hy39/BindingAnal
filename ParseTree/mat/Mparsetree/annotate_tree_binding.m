function [tr] = annotate_tree_binding(file, netcharge, bdingscr)
...
% read the viral trait: netcharge
dat = open(netcharge)
header_txt = dat.header_txt;
for i=1:length(header_txt(:,1))
  headercell(i) = deblank({header_txt(i,:)});
end
charge = dat.charge_txt;

% read the viral trait: binding score
dat = load(bdingscr);
binding_txt = dat.binding_txt;
...
%%Need to find the netcharge given internal ID. Jul 2, 2013
        ...
        trait_charge = num2str(charge(strcmp(num2str(cur_inter_node),headercell),1));  
        trait_binding = num2str(binding_txt(find(binding_txt==cur_inter_node),2));
        cur_inter_str = '';
        cur_inter_str = [num2str(cur_inter_node) '[&annotation="",Netcharge.set="' trait_charge '",Bindingscr.set="' trait_binding '"]'];
...
% Search the netcharge for cur_tip.
      cur_tip %disp       
      trait_charge = num2str(charge(strcmp(num2str(cur_tip),headercell),1));
      trait_binding = num2str(binding_txt(find(binding_txt==cur_inter_node),2));
      ...
      cur_tip_str = '';
      % add new annotation
      cur_tip_str = [num2str(nm(end,1:pos_e)) '[&annotation="",Netcharge.set="' trait_charge '",Bindingscr.set="' trait_binding '"]'];
...
outfile = [file(1:regexp(file, '\.')-1) '.alltraits' file(regexp(file, '\.'):end)];
