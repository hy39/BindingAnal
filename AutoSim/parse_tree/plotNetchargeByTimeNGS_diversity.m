...
for ng = ngs_no(1):ngs_no(end)
    ...
    % calculate the netcharge diversity
    charge_tr = charge_name(ngs_tr);
    A = charge_tr;
    %[fq x] = hist(A, genotype_list)./length(A(1,:));
    pix = 0;
    for i=2:length(genotype_list)
        for j=1:i-1
          pix = pix + x(i)*(xj)*abs(genotype_list(i)-genotype_list(j));
        end
    end
    pix = pix*2;
...
% Add the function to measure netcharge diversity
